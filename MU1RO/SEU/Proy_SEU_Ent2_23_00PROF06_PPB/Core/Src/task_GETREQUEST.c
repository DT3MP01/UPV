		...
		do {
			...
			if (COMM_request.command==0){ //nada quiere nada
				...
				if(STATE=0){
					...
					// MODO CONECTADO --- Comprobar que no se ha apagado la alarma
					 if(global_mode==1){

					 }
					 // Modo Clon -- Clonar los datos de la alarma
					 if(global_mode==2){
					...
					else{bprintf("Otro modo phase1\r\n");}
					STATE=(STATE+1)%2;
				}
				else{
					// Postear Los Datos del Sensor
					if(global_mode==1){
						COMM_request.command=1;
						COMM_request.result=0;
						COMM_request.type=global_mode;
