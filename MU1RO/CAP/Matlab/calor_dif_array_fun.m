function calor_dif_array_fun(nx)
dz=0.05;
dt=0.005;
q=2;
alpz=0.01;
...
function X = funaux(row, col)
    rowU=max(1,row-1);rowD=min(nx+2,row+1);
    colL=max(1,col-1);colR=min(nx+2,col+1);
    X=conaux(row,col)+dt*(conaux(rowU,col)+conaux(row,colL)-4*conaux(row,col)+conaux(rowD,col)+conaux(row,colR))*alpz/(dz*dz);
end
//rows y cols
