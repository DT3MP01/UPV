...
LLAA_ LLAC_ CORA_ CORC_ PARA_ PARC_ PUNTOCOMA_ COMA_ LOOP_ COUNTING_
...
Inst_Loop
        : LOOP_ {
                ...
                $<cent>$ = si; /*3*/
        }  instruccion  COUNTING_ { 

                emite(EDIF,crArgPos(niv, $<exp>$.desp),crArgEnt(1),crArgPos(niv, $<exp>$.desp));
        }  expresion { $<cent>$ = creaLans(si);
        emite(GOTOS,crArgNul(),crArgNul(),crArgEtq($<cent>2)); /*4*/}  {
                
                completaLans($<cent>2,crArgEtq(si));   /*1*/

                $<exp>$.desp = creaVarTemp();
                emite(EASIG,crArgPos(niv, $6.desp),crArgNul(),crArgPos(niv, $<exp>$.desp));

                completaLans($<cent>7,crArgEtq(si));   /*4*/

                $<cent>$ = creaLans(si);
                emite(EIGUAL,crArgPos(niv,$<exp>$.desp),crArgEnt(0),crArgEtq(-1)); /*2*/

                emite(GOTOS,crArgNul(),crArgNul(),crArgEtq($<cent>2)); /*3*/

        }  PUNTOCOMA_{completaLans($<cent>8,crArgEtq(si));  /*2*/} 
        ;
