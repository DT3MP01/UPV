...
import sys
import math
from SAR_ALT_spellsuggest import TrieSpellSuggester

# py .\SAR_Searcher.py -C -Q 'alexanderx' -E -D 'levenshtein' -H '1' 2015_index_full.bin
# py .\SAR_Indexer.py -S -P -M -O -U corpora/2016 2016_index_full.bin
#py .medirtiempos.py
...
class SAR_Project:
    ...
    def __init__(self):
        ...
        # self.index  {'title': {},'date': {},'keywords': {},'article': {queso :[123.],cuanto [1 2 3]},'summary': {}
    ...
    # ALT EXTENSION:Activar la busqueda aproximada para el searcher
    def set_search(self, s, dist, th):
        """
        Activa el modo de busqueda por sugerencia.
        input: "s" booleano,
               "dist" string,
               "th" int.
        si self.suggest es True se mostraran resultados aproximandolos a un threshold aplicando la Distancia 
        """
        self.use_search = s
        self.distance = dist
        self.threshold = th
    ...
    def index_dir(self, root, **args):
        ...
        # ALT -- Saving in self.spellsuggester the spellsuggester for each field
        # That is because we implement multifield.
        if self.suggest:
            for field in ['title', 'date',
                          'keywords', 'article', 'summary']:
                self.spellsuggester[field] = TrieSpellSuggester(
                    list(self.index[field].keys()))
        ##########################################
    ...
    def get_posting(self, term, field='article'):
        ...
        # ALT:si la lista de terminos esta vacia y self.search esta activada busca las sugerencias y añadelas al resultado
        if res == [] and self.use_search == True:
            # Distancias
            if self.distance in ["levenshtein", "restricted", "intermediate", ]:
                # print("hola",self.use_search,self.distance,self.threshold)
                suggestion = self.spellsuggester[field].suggest(
                    term, self.distance, self.threshold)
                for sugg_term in suggestion.keys():
                    res = self.or_posting(
                        res, self.get_posting(sugg_term, field))
            # TRIES
            elif self.distance in ["levenshtein-trie", "restricted-trie", "intermediate-trie"]:
                suggestion = self.spellsuggester[field].suggest(
                    term, self.distance, self.threshold)
                for sugg_term in set(item for item, item2 in suggestion):
                    res = self.or_posting(
                        res, self.get_posting(sugg_term, field))
        return res
