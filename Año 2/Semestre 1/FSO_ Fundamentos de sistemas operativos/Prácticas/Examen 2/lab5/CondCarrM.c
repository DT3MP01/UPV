...
*/
#define REPETICIONES      20000000 //produce condición de carrera (ambos hilos acceden a la vez)
...
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; //Declara e inicializa el mutex
...
void *agrega (void *argumento) {
  ...
  for (cont = 0; cont < REPETICIONES; cont = cont + 1) {
      pthread_mutex_lock(&mutex); //Sección de entrada
      V = V + 1;
      pthread_mutex_unlock(&mutex); //Sección de salida
  }
