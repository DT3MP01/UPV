...
*/
#define REPETICIONES      20000000 //produce condición de carrera
...
int llave = 0; //0 = False -> Sección crítica no está ocupada
...
void *agrega (void *argumento) {
  ...
  for (cont = 0; cont < REPETICIONES; cont = cont + 1) {
      while(test_and_set(&llave)); //Sección de entrada
      V = V + 1;
      llave = 0; //Sección de salida
  }
...
void *inspecciona (void *argumento) {
  ...
  for (;;) {
    usleep(200000); //Equivalente a: sleep(2)
