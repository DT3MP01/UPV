...
public class Ordenacion {
    ...
    private static <T extends Comparable<T>> void quickSort(T[] a,
        ...
        if (izq < der) {
            ...
            // System.out.println("He seleccionado el pivote: " + pivot);
            int i = izq;
            int j = der - 1;
            String s = "";
            while (i < j) {
            ...
            // System.out.println("He hecho una llamada con izq = " + izq + ", der = " + der);
            // s += "izq = " + izq + ", der = " + der;
    ...
    private static <T extends Comparable<T>> T mediana3(T[] a,
            int izq, int der) {
        int c = (izq + der) / 2;
        if (a[c].compareTo(a[izq]) < 0) {
            intercambiar(a, izq, c);
        }
        if (a[der].compareTo(a[izq]) < 0) {
            intercambiar(a, izq, der);
        }
        if (a[der].compareTo(a[c]) < 0) {
    ...
    /**
     * Mezcla internamente los subarrays v[i, m] y v[m + 1, f], ambos ordenados
     * Asc.
     *
     * @param v Sus elementos implementan la interfaz Comparable
    ...
    /**
     * SII i<=f: devuelve un array con los elementos del subarray v[i, f]
     * ordenados Asc.
     *
     * @param v Sus elementos implementan la interfaz Comparable
     * @param i Extremo inferior del intervalo a ordenar
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
            int i, int f) {
        if (i == f) {
            // El array solo tiene un elemento
            T[] aux = (T[]) new Comparable[1];
            aux[0] = v[i];
            return aux;
        } else if (i + 1 == f) {
            // El array solo tiene dos elementos
            ...
            T[] a, b;
            int m = (i + f) / 2;
            // System.out.println("Hago una llamada con i = " + i + ", m = " + m + "\n");
            a = mergeSort2(v, i, m);
            // System.out.println("Hago otra llamada con m + 1 = " + (m + 1) + ", f = " + f);
