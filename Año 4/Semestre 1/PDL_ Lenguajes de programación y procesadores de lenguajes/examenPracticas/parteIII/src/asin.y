...
instruccionIteracion2
        : FROM_ ID_ ASIG_ expresion {
                SIMB simb = obtTdS($2); /* Utilizamos simb para obtener las variables */
                if ($4.tipo != T_ERROR) {
                        if (simb.t == T_ERROR) {
                                yyerror("Objeto no declarado");
                        } else if (simb.t != $4.tipo && $4.tipo != T_ERROR) {
                                yyerror("Error de tipos en la asignación");
                        } 
                } 
                emite(EASIG, crArgPos(niv, $4.desp), crArgNul(), crArgPos(simb.n, simb.d)); /* asignar expresion al ID */
                $<cent>$ = si;
        } TO_ expresion {
                SIMB simb = obtTdS($2);
                $<cent>$ = creaLans(si);    
                emite(EMAYEQ, crArgPos(niv, $7.desp), crArgPos(simb.n, simb.d), crArgEtq(-1));
        } DO_ instruccion {
                SIMB simb = obtTdS($2);
                emite(ESUM, crArgPos(simb.n, simb.d), crArgEnt(1), crArgPos(simb.n, simb.d)); /* incrementamos el ID */
                emite(GOTOS, crArgNul(), crArgNul(), crArgEtq($<cent>5)); /* saltamos al inicio del bucle */
                completaLans($<cent>8, crArgEtq(si)); /* completa la lista, tiene que saltar fuera */
        }
        ;
