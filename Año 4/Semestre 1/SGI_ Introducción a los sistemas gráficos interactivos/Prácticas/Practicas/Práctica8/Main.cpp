...
#include <sstream>
constexpr auto PROYECTO = "Circuito texturado";
constexpr auto PI = 3.1415926;
constexpr auto tasaFPS = 60;

static enum class MODE { SOLIDO, Wired } modo1;
static enum class DAY_TIME { DIA, Night } modo2;
static float speed = 0, girox = 0, giroz = -1;
static float distx = 0, disty = 1, distz = 0;
static float avance = 0.0;
static float angle = 180;
static GLuint suelo, cartel1, cartel2, background_texture, plano, meta;
...
void display()
{
	...
	GLfloat L1_position[] = { 0, 0.7f, 0, 1 };  // Light from car headlamps
	glLightfv(GL_LIGHT1, GL_POSITION, L1_position);
	GLfloat dir_light1[] = { 0, -0.5f, -0.7f };
	glLightfv(GL_LIGHT1, GL_SPOT_DIRECTION, dir_light1);

	gluLookAt(distx, disty, distz, (double) distx + (double) girox, 1, (double) distz + (double) giroz, 0, 1, 0);

	if (modo2 == DAY_TIME::Night) {
...
void onArrow(int tecla, int x, int y)
{
	...
	giroz = cos(angle * PI / 180);
	girox = sin(angle * PI / 180);
