...
__global__ void crec_gpu1(int *A,  int *sal)
{
  int i,j,res=1; 
  int tid=threadIdx.x;
 ...
 __global__ void crec_gpuv2(int *A, int *sal)
{
...
if (salcpu==1)
printf(" \n La matriz es creciente por filas");
else
 printf(" \n La matriz no es creciente por filas");
...
//crec_gpu1 <<<1,N>>>(dev_A, dev_sal);
crec_gpuv2<<<N,BLOCKSIZE>>>(dev_A, dev_sal);
