...
void mexFunction(int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs!=2)
       mexErrMsgIdAndTxt("Matlab:solve_tril_unidad_mex:nrhs","Two inputs required.");

    if (nlhs!=1)
      mexErrMsgIdAndTxt("Matlab:solve_tril_unidad_mex:nlhs","One output required.");
    ...
    if (mA!=nA)
        mexErrMsgIdAndTxt("Matlab:solve_tril_unidad_mex:size","Square matrix required as input");   
