...
/* Resuelve un sistema de ecuaciones Ax=b, siendo A una matriz banda triangular
...
void solve_triu_banda_mex(double *A, double *b,int n,int ancho)
{
    /* Cuidado que la variable i no sea de tipo size_t o mwSize (ya que
       almacenan enteros sin signo) y el código no irá bien cuando i valga
       -1 */
    int i,j; 
    /* Versión 1, por filas */
    for (i=n-1;i>=0;i--) {
        for (j=i+1;j<=minimo(i+ancho,n-1);j++) {
            b[i]=b[i]-A[i+n*j]*b[j]; // Notación C
            //b(i)=b(i)-A(i,j)*b(j); // Notación Matlab
        }
        b[i]=b[i]/A[i+n*i]; // Notación C
        //b(i)=b(i)/A(i,i); // Notación Matlab
    }        
}
...
void mexFunction(int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[])
{
    ...
    size_t mb,nb; /* Dimensión de la matriz */
    size_t ancho; /* Ancho de banda */    
    double *lu;   /* Matriz de salida */
    int i;
    /* check for proper number of arguments */
    if (nrhs!=2)
       mexErrMsgIdAndTxt("Matlab:solve_triu_banda_mex:nrhs","Two inputs required.");

    if (nlhs!=1)
      mexErrMsgIdAndTxt("Matlab:solve_triu_banda_mex:nlhs","One output required.");
    ...
    if (mA!=nA) 
        mexErrMsgIdAndTxt("Matlab:solve_triu_banda_mex:size","Square matrix required as input");       
    ...
    ancho=sqrt(nA);
    solve_triu_banda_mex(A,x,nA,ancho);
}
