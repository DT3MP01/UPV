
//Ejercicio de recapitulación (operaciones asíncronas y clausuras):
//     - ¿Cuál es el resultado de la ejecución de este código?
//     - Una vez contestada la cuestión anterior ejecute el código 
//       compruebe y la validez de la respuesta.



for (var i = 0; i < 5; i++)
   (function () {
      setTimeout(function () { console.log("---> ", i); }, i * 1000); // devolverá 5 siempre, ya que la i es global y no se guarda
   })();


for (var k = 0; k < 3; k++)
   (function (k) { // imprimirá k = 0, k = 1, k = 2
      setTimeout(
         function () { console.log("==========> ", k); }, k * 4010);
   })(k);


for (var x = 0; x < 3; x++) {
   (function () {
      var mm = x;
      setTimeout(function () {
         var m = x;
         console.log("::::::::::::::> ", m, "   ", mm);
      }, x * 8030);
   })();
}