                   ...
                   int me, int np)
{
  ...
  if (me == 0)
  {
    ...
    for (rows_done = 0; rows_done < h; rows_done++)
    {
      ...
      MPI_Irecv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &req);

      MPI_Test(&req, &finished, &status);

      while (!finished && rows_done < h)
      {
        z0.b = y1 + iy * next_row;
        for (j = 0; j < w; j++)
        {
...
int main(int argc, char *argv[])
{
  ...
  /* Lo que sigue es para sacar el m�ximo valor permitido como TAG */
  ...
  if (np < 2 || (flag && H - 1 > *max_tag))
  {
    ...
    {
      fprintf(stderr,
              "No se puede usar el TAG para indicar el n�mero de fila (no cabe).\n");
    }
    MPI_Finalize();
    return 1;
  }
  ...
  while (argc > 1)
  {
    if (argv[1][0] == '-')
    {
      ...
      case 'i':
        ...
        /* Ojo! maxiter no debe valer mas de 255 */
        if (maxiter > 255)
          maxiter = 255;
        break;
      case 'r':
