...
int lu(double **A, int n, int iproc, int p, int mb)
{
  ...
  for (k = 0; k < n - 1; k++)
  {
    ...
    {
      if (fabs(A[localIndex(k, p, mb)][k]) < EPSILON)
        return -1;
...
int triSup(double **U, double *b, int n, int iproc, int p, int mb)
{
  ...
  for (i = n - 1; i >= 0; --i)
  {
    if (owner(i, p, mb) == iproc)
    {
      if (fabs(U[localIndex(i, p, mb)][i]) < EPSILON)
        return -1;
...
int main(int argc, char *argv[])
{
  ...
  {
    if (iproc == 0)
      printf("Error in the system size: %d\n", n);
    MPI_Finalize();
    return 0;
  }
  ...
  for (i = 0; i < mb; ++i)
  {
    MPI_Scatter(&A[i * p][0], n, MPI_DOUBLE, &Aloc[i][0], n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  }
