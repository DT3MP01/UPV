...
public class Point {
    ...
    public double distance(Point p) { 
        double abs = p.x - x; double ord = p.y - y; 
    ...
    public int cross(Point u, Point v) {        
        ...
        if (u.y < v.y) { pLow = u; pHigh = v; }
        else { pLow = v; pHigh = u; }
        /* COMPLETAR: asignar un valor a cut siguiendo el analisis 
         * de casos del boletin */
         
        int cut = DONT_CROSS; // Hipotesis: (pLow.y == pHigh.y)
        if (pLow.y != pHigh.y) { 
            if (y == pHigh.y && x <= pHigh.x) { cut = HIGH_CROSS; }
            else {
                if (y == pLow.y && x <= pLow.x) { cut = LOW_CROSS; }
                else {  
                    double yCut = y;
                    double xCut = (y - pLow.y) * (pHigh.x - pLow.x) 
                                  / (pHigh.y - pLow.y) + pLow.x;
                    if (x <= xCut && pLow.y < yCut && yCut < pHigh.y) { 
                        cut = CROSS; 
                    } 
                    // else { cut = DONT_CROSS; }
                }
