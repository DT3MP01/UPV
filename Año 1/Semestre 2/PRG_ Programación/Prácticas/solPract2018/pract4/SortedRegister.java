    ...
    public void add(Scanner s, PrintWriter err) {
        ...
        while (s.hasNext()) {
            try {
                ...
                int amount = s.nextInt();
                if (amount < 0) {
                    throw new IllegalArgumentException("El valor ha de ser positiu");
                }
                this.m[month][day] += amount;
            } catch (IndexOutOfBoundsException e) {
                err.printf("Línea %d: Dato incorrecto\n", linea);
                s.nextLine();
            } catch (IllegalArgumentException e) {
                err.printf("Línea %d: Cantidad negativa\n", linea);
                s.nextLine();
            } catch (InputMismatchException e) {
                err.printf("Línea %d: Formato incorrecto\n", linea);
                s.nextLine();
            }
            linea++;
        }
